...
import RealisticFlower from '../3d/RealisticFlower';
import FloatingParticles from '../3d/FloatingParticles';
import ErrorBoundary from '../ErrorBoundary';

// Animation for the central rose
const AnimatedRose = ({ position = [0, 0, 0] }: { position?: [number, number, number] }) => {
  const roseRef = useRef<THREE.Group>(null);
  
  useFrame(({ clock }) => {
    if (roseRef.current) {
      // Gentle floating animation
      roseRef.current.position.y = position[1] + Math.sin(clock.getElapsedTime() * 0.5) * 0.1;
      // Slow rotation
      roseRef.current.rotation.y = clock.getElapsedTime() * 0.15;
    }
  });
  
  return (
    <group ref={roseRef} position={position as [number, number, number]}>
      <RealisticFlower
        position={[0, 0, 0]}
        rotation={[0, 0, 0]}
        scale={1.2}
    ...
    <Dialog.Root>
      <Tooltip.Provider delayDuration={300}>
        <section className="relative min-h-[70vh] bg-gradient-to-b from-gray-900 to-primary-900 py-20 overflow-hidden">
          {/* Background flowers canvas */}
          <div className="absolute inset-0">
            <ErrorBoundary fallback={
              <div className="flex items-center justify-center h-full bg-gradient-to-b from-gray-900 to-primary-900">
                <p className="text-white text-lg">3D content could not be loaded</p>
              </div>
            }>
              <Canvas shadows dpr={[1, 2]}>
                <ambientLight intensity={0.5} />
                <spotLight position={[0, 5, 5]} angle={0.25} penumbra={1} intensity={1.5} castShadow />
                <pointLight position={[-10, -10, -10]} intensity={0.5} />
                <pointLight position={[10, 10, 10]} intensity={0.3} />
                
                <Suspense fallback={null}>
                  {/* Central animated rose */}
                  <AnimatedRose position={[0, 0, -1]} />
